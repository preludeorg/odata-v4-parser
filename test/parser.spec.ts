...
describe('Parser', () => {
  ...
  it('should instantiate odata parser', () => {
    ...
    expect(
      ast?.value.value.value.value.next.value.value.predicate.value.value.right.value
    ).toEqual('Edm.String');
  });
  ...
  it('should parse custom query options', () => {
    ...
    expect(ast?.value.options[0].type).toBe('CustomQueryOption');
    expect(ast?.value.options[1].type).toBe('CustomQueryOption');

    if (isType(ast?.value.options[0], 'CustomQueryOption')) {
      expect(ast?.value.options[0].value.key).toEqual('foo');
      expect(ast?.value.options[0].value.value).toEqual('123');
    }

    if (isType(ast?.value.options[1], 'CustomQueryOption')) {
