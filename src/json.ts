...
export function collectionPropertyInUri(value: SourceArray, index: number): Lexer.Token {
...
export function primitiveColInUri(value: SourceArray, index: number): Lexer.Token {
  const begin = Lexer.beginArray(value, index);
  if (begin === index) { return; }
  const start = index;
  index = begin;
...
export function complexPropertyInUri(value: SourceArray, index: number): Lexer.Token {
  let mark = Lexer.quotationMark(value, index);
  if (mark === index) { return; }
  const start = index;
  index = mark;
...
export function annotationInUri(value: SourceArray, index: number): Lexer.Token {
  let mark = Lexer.quotationMark(value, index);
  if (mark === index) { return; }
  const start = index;
  index = mark;
...
export function keyValuePairInUri(value: SourceArray, index: number, keyFn: Function, valueFn: Function): Lexer.Token {
  let mark = Lexer.quotationMark(value, index);
  if (mark === index) { return; }
  const start = index;
  index = mark;
...
export function primitivePropertyInUri(value: SourceArray, index: number): Lexer.Token {
  return keyValuePairInUri(value, index, NameOrIdentifier.primitiveProperty, primitiveLiteralInJSON);
}

export function navigationPropertyInUri(value: SourceArray, index: number): Lexer.Token {
  return singleNavPropInJSON(value, index) ||
    collectionNavPropInJSON(value, index);
}

export function singleNavPropInJSON(value: SourceArray, index: number): Lexer.Token {
  return keyValuePairInUri(value, index, NameOrIdentifier.entityNavigationProperty, Expressions.rootExpr);
}

export function collectionNavPropInJSON(value: SourceArray, index: number): Lexer.Token {
...
export function primitiveLiteralInJSON(value: SourceArray, index: number): Lexer.Token {
